...
num_runs = 1000
results_filename = "sources_found_comparison.pkl"
results = []


def get_num_source_found(env):
    return np.sum(env._hidden_state[LOCATED_SOURCES])
...
for desc, agent in agents_to_compare:
    ...
    for _ in range(num_runs):
        ...
        num_found.append(get_num_source_found(env))
    mean = np.mean(num_found)
    std = np.std(num_found)
...
fig, ax = plt.subplots()
ax.bar(descs, means, yerr=stds, alpha=0.5, ecolor='black', capsize=10)
ax.set_ylabel("Mean Number of Sources Found\n(with std. dev.)")
ax.set_title("Average Number of Sources Found by Agent Algorithms")
ax.yaxis.grid(True)
plt.xticks(rotation='vertical')
plt.tight_layout()
plt.savefig("./data/sources_found_comparison_figure.png")
plt.show()
